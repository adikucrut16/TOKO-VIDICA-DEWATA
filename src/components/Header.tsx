...
import { Menu, RefreshCw, Calendar, Trash2, Sliders } from 'lucide-react';
        ...
        <button
          ...
          className={`flex items-center gap-2 text-xs px-3.5 py-1.5 rounded-xl border transition-all ${
            sheetsConfig.webAppUrl
              ? 'bg-indigo-50 text-indigo-600 border-indigo-200 hover:bg-indigo-100/80 font-semibold'
              : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200/70 font-medium'
          }`}
          title={sheetsConfig.webAppUrl ? 'Sinkronisasi ke Google Sheets' : 'Atur URL Google Sheets di Pengaturan'}
        >
          <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin text-indigo-600' : ''}`} />
          <span className="hidden sm:inline">
            {isSyncing ? 'Syncing...' : sheetsConfig.webAppUrl ? 'Sync Sheets' : 'Atur Sync'}
          </span>
        </button>
